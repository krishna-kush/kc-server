        ...
        App::new()
            ...
            .service(
                web::scope("/auth")
                    ...
                    .route("/signup", web::post().to(handlers::verify_otp_and_signup))
                    .route("/google/config", web::get().to(handlers::get_google_config))
                    .route("/google/callback", web::post().to(handlers::google_callback))
            )
            ...
            .service(
                web::scope("/settings")
                    ...
                    .route("/security", web::get().to(handlers::get_security_settings))
                    .route("/security/2fa", web::post().to(handlers::toggle_2fa))
                    .route("/security/add-password", web::post().to(handlers::add_password))
                    .route("/security/change-password", web::post().to(handlers::change_password))
            )
            // Public password reset endpoints (no auth required)
            .route("/auth/request-password-reset", web::post().to(handlers::request_password_reset))
            .route("/auth/reset-password", web::post().to(handlers::verify_reset_otp))
            .service(
                web::scope("/protected")
                    .wrap(middleware::AuthMiddleware)
